...
在 PT 站点用户详情页，一键认领全部当前做种种子（支持分页翻页、折叠列表自动展开、可视化进度、已认领去重跳过）。
...
- 自动收集当前做种列表**所有未认领**种子：
  - 分页站点自动翻遍所有分页（KamePT）
  - 折叠列表站点**自动展开/加载**，无需手动点「显示/隐藏」（NicePT、PTFans）
- 对每个未认领的做种种子通过 `ajax.php` 发送 `addClaim` 请求
- 每个种子间隔 500ms，避免短时间多次请求被处理
- **可视化进度面板**：右上角浮动面板实时显示进度条、当前/总数、成功/失败数
- **已认领去重**：认领成功的种子 id 按站点记录到 localStorage，下次自动跳过，不再重复认领
